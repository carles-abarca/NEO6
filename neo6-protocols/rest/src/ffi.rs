// FFI implementation for REST protocol
...
struct RestFFIHandler {
    handler: Arc<RestHandler>,
    runtime: Arc<Runtime>,
}

/// Create a new REST protocol handler instance
extern "C" fn rest_create_handler() -> ProtocolHandle {
    match Runtime::new() {
        Ok(runtime) => {
            let handler = Arc::new(RestHandler::new("http://localhost:8080".to_string(), None));
            let ffi_handler = Box::new(RestFFIHandler {
                handler,
...
/// Destroy a REST protocol handler instance
extern "C" fn rest_destroy_handler(handle: ProtocolHandle) {
    if !handle.is_null() {
        unsafe {
            let _ = Box::from_raw(handle as *mut RestFFIHandler);
        }
    }
}

/// Invoke a transaction on the REST protocol handler
unsafe extern "C" fn rest_invoke_transaction(
    ...
    let ffi_handler = unsafe { &*(handle as *const RestFFIHandler) };
...
pub unsafe extern "C" fn get_protocol_interface() -> *const ProtocolInterface {
    static INTERFACE: ProtocolInterface = ProtocolInterface {
        create_handler: rest_create_handler,
