// FFI implementation for TCP protocol
...
struct TcpFFIHandler {
    handler: Arc<TcpHandler>,
    runtime: Arc<Runtime>,
}

/// Create a new TCP protocol handler instance
extern "C" fn tcp_create_handler() -> ProtocolHandle {
    match Runtime::new() {
        Ok(runtime) => {
            let handler = Arc::new(TcpHandler);
            let ffi_handler = Box::new(TcpFFIHandler {
                handler,
...
/// Destroy a TCP protocol handler instance
extern "C" fn tcp_destroy_handler(handle: ProtocolHandle) {
    if !handle.is_null() {
        unsafe {
            let _ = Box::from_raw(handle as *mut TcpFFIHandler);
        }
    }
}

/// Invoke a transaction on the TCP protocol handler
unsafe extern "C" fn tcp_invoke_transaction(
    ...
    let ffi_handler = unsafe { &*(handle as *const TcpFFIHandler) };
...
/// Set logging level for TCP protocol
unsafe extern "C" fn tcp_set_log_level(log_level: *const c_char) -> FfiResult {
...
/// Get the protocol interface for TCP (only exported function)
#[unsafe(no_mangle)]
pub unsafe extern "C" fn get_protocol_interface() -> *const ProtocolInterface {
    static INTERFACE: ProtocolInterface = ProtocolInterface {
        create_handler: tcp_create_handler,
        destroy_handler: tcp_destroy_handler,
        invoke_transaction: tcp_invoke_transaction,
        start_listener: None,
        set_log_level: Some(tcp_set_log_level),
    };
    &INTERFACE
}
